...
# คำลงท้ายของ **ภาคนี้** คือ ขอรับ / เจ้าค่ะ / เจ้าคะ (คำตัดสิน lead 2 ก.ย. 2026)
# ⚠ รูปเดิมที่ยกมาจาก Lost Judgment จับแต่ ครับ/ค่ะ ซึ่งภาคนี้ห้ามใช้ในบทพูดอยู่แล้ว
#   ผลคือตัวตรวจ "วัดอะไรไม่ได้เลย" และเงียบสนิททั้งที่ควรเตือน (ผู้ตรวจ batch_001 จับได้)
import thai_pronouns as _tp        # noqa: E402
PARTICLE_RE = _tp.POLITE_OLD
# คำลงท้ายของภาคปัจจุบันที่หลุดเข้ามาในบทพูดยุคเก่า — นับแยก ควรเป็น 0 เสมอ
MODERN_PARTICLE_RE = re.compile(_tp.RE_KHRAP.pattern + r"|" + _tp.RE_KHA_MODERN.pattern)
...
def load_tiers():
    ...
    out = {}
    # ภาคนี้เก็บทะเบียนตัวละครไว้ที่ `characters.json` (สร้างโดย build_characters.py)
    # ชื่อไฟล์ของ Lost Judgment ยังอ่านเผื่อไว้ เพื่อให้สคริปต์นี้ใช้ข้ามโปรเจกต์ได้
    for name in ("characters.json", "characters_main.json", "characters_side.json"):
        p = paths.TRANSLATIONS / name
        if not p.exists():
            continue
        data = json.load(io.open(p, encoding="utf-8"))
        # `characters.json` ของภาคนี้ซ้อนอีกชั้น: {"main": {...}, "other_speakers": {...}}
        if data and all(isinstance(v, dict) and "gender" not in v for v in data.values()):
            merged = {}
...
def analyse(batch, parts):
    ...
    # ไฟล์บริบทของภาคนี้ห่อข้อมูลไว้ใต้คีย์ `lines` (ของ LJ เป็น dict ชั้นเดียว)
    ...
    for i, k in enumerate(keys):
        ...
        if rec.get("neutral"):
            skipped["neutral"] += 1
        elif MONOLOGUE_RE.search(k) or MONOLOGUE_RE.search(th):
            skipped["monologue"] += 1
        # ไฟล์บริบทของภาคนี้เก็บผู้พูดเป็น **รายการ** `speakers` ไม่ใช่ช่อง `speaker` เดี่ยวแบบ LJ
        # ถ้าไม่รองรับตรงนี้ ตัวตรวจจะข้ามทุกบรรทัดแล้วเงียบสนิท (เจอ 2 ก.ย. 2026 กับ batch_001)
        # บรรทัดที่มีผู้พูดหลายคนไม่ผูกกับใครคนเดียว จึงไม่นับเข้าความหนาแน่นรายตัวละคร
        spk = rec.get("speaker")
        if not spk:
            # แต่ละรายการเป็น dict {"name": ..., "gender": ..., "uses": ...}
            sp = [s.get("name") for s in (rec.get("speakers") or [])
                  if isinstance(s, dict) and str(s.get("name") or "").strip()]
            spk = sp[0] if len(sp) == 1 else ""
        spk = str(spk).strip()
        if not spk and not rec.get("neutral"):
            skipped["ไม่ผูกกับผู้พูดคนเดียว"] += 1
...
def main():
    ...
    for b in batches:
        ...
        for spk, items in sorted(per.items(), key=lambda x: -len(x[1])):
            ...
            warned += bad
        if not rows:
            # เดิมเงียบสนิทเมื่อไม่มีแถว ทำให้แยกไม่ออกว่า "ผ่าน" หรือ "ตรวจไม่ได้"
            print("== batch_%s: ไม่มีตัวละครที่วัดได้ (บทที่ต้องกลางเพศ %d · มอนอโลก %d · "
                  "ไม่ผูกกับผู้พูดคนเดียว %d · ที่เหลือบทน้อยกว่า %d บรรทัด)"
                  % (b, skipped["neutral"], skipped["monologue"],
                     skipped["ไม่ผูกกับผู้พูดคนเดียว"], a.min_lines))
            continue
