...
#   **んだわ = ชาย** (12 ก.ย. 2026): เจ้าของร้านโชกิพูด "…ダメなんだわ" ทั้งบทสอน แต่ข้อความสายสัมพันธ์
#   ของเขาเขียนว่า "interact with **him**" · กวาดทั้งคลัง んだわ 30 บรรทัด มาจากบทเดียวกันซ้ำ 5 ไฟล์เท่านั้น
#   -> เติม (?<!ん) กัน だわ ที่เป็นรูปอธิบาย んだ+わ ของชาย (ผู้ตรวจ subagent จับได้ตอนคลื่นเพศผู้พูด)
#   **わたくし = ไม่บอกเพศ** (12 ก.ย. 2026): กวาดทั้งคลังมี 21 บรรทัด · 15 บรรทัดเป็นบทสามพี่น้อง
#   ตระกูลโมโมกาวะ (uid000c14bc) ที่เรียกตัวเองว่า 長男/次男/三男 และใช้ ボク ด้วย = ชายชัดเจน
#   เหลือฝั่งหญิงแค่ 6 บรรทัดของอาจารย์ไมโจ ซึ่งมี かしら/わよ อยู่แล้ว -> เอา わたくし ออกจาก regex
#   ชุดที่เหลือ: ชาย 3 หญิง 28 (สามจุดที่เหลือเป็นป้ายผู้พูดที่ทะเบียนเพศผิด ไม่ใช่บั๊ก regex)
JA_FEMALE_RE = re.compile(r"(?<!お)かしら(?:って|[んね]?(?=[。！？…、」』〜～ーぁぃぅぇぉ\r\n]|$))|ですもの|あたし|アタシ|あたい|(?<!ます)(?<!です)(?<!した)わよ(?=ね|[。！？…、」』〜～ーぁぃぅぇぉ\r\n]|$)|(?<!ます)(?<!です)(?<!した)(?<![一-鿿])わね(?=[。！？…、」』〜～ーぁぃぅぇぉ\r\n]|$)|(?<!こ)(?<!ん)だわ(?=[。！？…、」』〜～ーぁぃぅぇぉ\r\n]|$)|(?<![んンでだ])のよ(?=[。！？…、」』〜～ーぁぃぅぇぉ\r\n]|$)")
...
_DIALOGUE_GENDER = None


def dialogue_gender(en):
    """เพศผู้พูดที่ผู้ตรวจอ่านบทสนทนา EN คู่ JA แล้วชี้ พร้อมหลักฐานที่ตรวจซ้ำด้วยเครื่องแล้ว

    สร้างด้วย `scripts/make_gender_packets.py` + `scripts/merge_gender_wave.py` (12 ก.ย. 2026)
    ทุกคีย์ผ่านด่าน: หลักฐานต้องเป็นข้อความที่มีอยู่จริงในบรรทัดของไฟล์ฉากเดียวกัน และห้ามเป็น
    รูปสำเนียงคันไซที่วัดแล้วว่าไม่บอกเพศ (ですわ · ますわ · わ · ぜよ · どす)

    วัดกับซองควบคุมแล้ว: ตรงกับเครื่องหมายในบรรทัดเอง (ja_gender) 220/220 = 100%
    ที่ขัดกับ scene_gender คือเคสที่ scene_gender ตีทั้งฉากเป็นเพศเดียวแล้วพลาดตัวละครอีกเพศ

    ชั้นนี้อยู่ **ใต้** `ja_gender` (เครื่องหมายในบรรทัดเอง) แต่ **เหนือ** `scene_gender` (ระดับไฟล์)
    """
    global _DIALOGUE_GENDER
    if _DIALOGUE_GENDER is None:
        p = paths.TRANSLATIONS / "gender_dialogue.json"
        raw = json.loads(p.read_text(encoding="utf-8")) if p.exists() else {}
