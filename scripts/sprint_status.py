...
def batch_ids():
    ...
    # คำที่ก้อนก่อนเคาะแล้ว (.prior.json) · ผลค้นของที่ ship แล้ว (.priorart.json · .skillart.json)
    for p in sorted(b for b in paths.WORKLIST.glob("batch_*.json")
                    if not b.name.endswith((".context.json", ".prior.json",
                                            ".priorart.json", ".skillart.json",
                                            ".dnt.json", ".todo.json"))):
...
def main():
    ...
    for p in (b for b in paths.WORKLIST.glob("batch_*.json")
              if not b.name.endswith((".context.json", ".prior.json", ".priorart.json",
                                      ".skillart.json", ".dnt.json", ".todo.json"))):
