...
ใช้:
  ...
  python scripts/check_alignment.py --self-test     # พิสูจน์กับเคสจริงของ b124 + เคสเลื่อนจำลอง
...
def check_file(path, verbose=False):
    ...
    global_bad = r["corr"] < MIN_CORR or r["diff"] > SHIFT_WINS
...
def self_test():
    src = os.path.join(DONE, "batch_124.done.json")
    if not os.path.exists(src):
        print("ไม่พบ batch_124.done.json — ข้าม self-test")
        return 0
    ...
    print("== 1) batch_124 ของจริง (ต้องผ่าน) ==")
    r = _judge(keys, vals)
    ...
    print("   ผล:", "ผ่าน" if ok1 else "!! ตรวจผิด — ไฟล์จริงถูกกล่าวหา")

    print("\n== 2) จำลองเคส b124 จริง — ทำคีย์หล่นที่ลำดับ 50 (คำแปล 200 บรรทัดเลื่อนหมด) ==")
    bk = keys[:50] + keys[51:]
    ...
    print("\n== 3) เคสยากกว่า — เลื่อนแค่ 40 คีย์ท้ายไฟล์ ==")
    bk3 = keys[:210] + keys[211:]
    bv3 = vals[:210] + vals[210:-1]
    r3 = _judge(bk3, bv3)
    w3 = _locate(bk3, bv3)
