...
def nchars(text):
    """จำนวน code point ของสตริงดิบ (นับแท็กด้วย) — `\r\n` นับเป็นหนึ่ง

    ⚠ **ไม่ใช่ตัวเลขที่เกมเก็บในบล็อกคำสั่ง** ใช้ `disp_chars()` สำหรับตำแหน่งตัวอักษร
    เหลือไว้เพราะ `scripts/sweep_len_fields.py` ยังอ้างอยู่
    """
    return len(text.replace("\r\n", "\n"))


def disp_chars(text):
    """จำนวนตัวอักษร "ที่วาดบนจอ" — ตัดแท็กทิ้ง ยกเว้น `<symbol=...>` กับ `<Sign:...>` ที่นับเป็นหนึ่ง

    นี่คือหน่วยที่บล็อกคำสั่งใช้จริง พิสูจน์กับ vanilla ทั้งชุด (11 ก.ย. 2026):
    ตำแหน่งสูงสุดในบล็อกคำสั่ง == ค่านี้ **65,853 จาก 65,855 บรรทัด** (คลัง en + ja)
    สองบรรทัดที่เหลือมีแท็ก `<%download_content%>` ที่เกมแทนด้วยข้อความจริงตอนรัน
    (ก่อนหน้านี้เข้าใจผิดว่าเป็น code point ของสตริงดิบ — บรรทัดที่มีแท็กจึงคำนวณผิดหมด)
    """
    return len(_strip_tags(text.replace("\r\n", "\n")))


def _strip_tags(s):
    return TAG_RE.sub(lambda m: "\x01" if m.group(0).startswith(("<symbol", "<Sign")) else "", s)


def _tag_marks(text):
    """ตำแหน่ง (หน่วย disp_chars) ของ *ขอบแท็ก* แต่ละตัวตามลำดับที่ปรากฏ

    ใช้จับคู่ตำแหน่งเดิม -> ตำแหน่งใหม่แบบตรงตัว: `<Color:8>` ในคำแปลอยู่คนละตำแหน่ง
    กับต้นฉบับ แต่เป็นแท็ก "ตัวที่ i" เหมือนกัน
    """
    s = text.replace("\r\n", "\n")
    out, seen = [], 0
    pos = 0
    for m in TAG_RE.finditer(s):
        seen += len(_strip_tags(s[pos:m.start()]))
        out.append(seen)
        if m.group(0).startswith(("<symbol", "<Sign")):
            seen += 1
        pos = m.end()
    return out
...
def retime_cmds(cmds, old_text, new_text):
    ...
    ทุกคำสั่งเก็บ "ตำแหน่งตัวอักษรในบรรทัด" ที่ไบต์ [6:8] (0 = ไม่ใช้) นับเป็น
    **ตัวอักษรที่วาดบนจอ ไม่นับแท็ก** (`disp_chars`) — ค่าที่เท่ากับความยาวเดิมคือจุดจบบรรทัด
    (เกมแสดง/เล่นเสียงถึงตรงนั้นแล้วหยุด) ค่าที่น้อยกว่าคือจุดกลางบรรทัด

    การจับคู่ตำแหน่ง:
      * จุดจบ (`v >= ความยาวเดิม`) -> ความยาวใหม่
      * ตำแหน่งที่ตรงกับขอบแท็ก -> ขอบแท็กตัวเดียวกันในคำแปล (ตรงตัว ไม่ใช่ประมาณ)
      * ที่เหลือ (จังหวะพิมพ์ · จุดหยุด) -> สเกลตามสัดส่วน

    ถ้าไม่ปรับ ข้อความไทยที่ยาวกว่าอังกฤษจะถูกตัดท้ายบนจอ
    (บั๊กจริง: "ไม่ได้เจอกันนานเลยนะ ซากาโมโตะซัง" 33 ตัว โชว์แค่ 28 ตัวเท่าประโยคอังกฤษ)

    ⚠ บทเรียน 11 ก.ย. 2026: รอบแรกนับด้วย code point ของสตริงดิบ ทำให้บรรทัด
    ที่มีแท็ก (`<Color:8>` · `<i>`) หาจุดจบไม่เจอเลย — ไปเข้าทางสเกลตามสัดส่วนแทน
    ผลคือ "ได้รับ <Color:8>โมเดล 2<Color:Default> แล้ว" ได้จุดจบ 18 ทั้งที่ต้องเป็น 19
    บนจอจึงขึ้น "ได้รับ โมเดล 2 แล้" — ตัว ว หายไปหนึ่งตัว (343 บรรทัดในบิลด์ v1.3)
    """
    od, nd = disp_chars(old_text), disp_chars(new_text)
    if od == 0:
        return list(cmds)
    exact = {}
    to, tn = _tag_marks(old_text), _tag_marks(new_text)
    if len(to) == len(tn):
    ...
    for c in cmds:
        ...
        if v >= od:
            ...
            nv = max(1, round(v * nd / od))
        nv = max(0, min(nv, 0xFFFF))
